...
import type { ChipFieldConfig } from '../../types/form';
...
export const ChipField = React.memo<ChipFieldProps>(
  ({ config, value = [], onChange, error, disabled }) => {
    ...
    const suggestions = useMemo(() => {
      ...
      return config.options.filter(
        (option) => option.toLowerCase().includes(searchTerm) && !value.includes(option)
      );
    ...
    return (
      <div className="w-full">
        ...
        <div className="space-y-2">
          ...
          {value.length > 0 && (
            <ul
              className="flex flex-wrap gap-2 mt-2 list-none p-0"
